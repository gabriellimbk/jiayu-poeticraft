...
- Student access: client-only LMS gate using `/?id=${Canvas.user.loginId}`.
...
10. If the LMS `id` query parameter is missing and the user is not an authenticated teacher, the app shows an Access Required screen and blocks app routes.
...
## Known Tradeoff
The LMS `?id=` access flow is client-only by requirement. That identifies students for UX and submission tracking, but it is not cryptographic authentication. Do not treat student-submitted identity as proof of identity unless the LMS link is later signed and verified server-side.
